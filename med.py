# STREAMLIT AI POWERED REMOTE PATIENT MONITORING APP(RPM) FOR GOOD HEALTH AND WELLBEING (Large-Scale Version)
    ...
    for user in user_ids:
        ...
        for _ in range(minutes):
            data.append({
                ...
                'medication_taken': np.random.choice([0, 1])  # 0: not taken, 1: taken
            })
...
# -----------------------------
st.header("🩺 Real-Time Monitoring Dashboard for Clinicians")
st.info("This section simulates real-time patient monitoring for anomaly detection.")

placeholder = st.empty()
for i in range(20):  # Simulate streaming data updates
    live_df = simulate_health_data(num_users=3, minutes=1)
    live_processed, live_scaled, _ = preprocess_data(live_df)
    live_preds, _ = detect_anomalies(live_scaled, contamination=contamination)  # <-- FIXED LINE
    live_df['anomaly'] = ['Anomaly' if x == -1 else 'Normal' for x in live_preds]
    
    
    # Count anomalies per user
    alert_counts = live_df[live_df['anomaly'] == 'Anomaly'].groupby('user_id').size()
    alert_users = alert_counts[alert_counts >= 3].index.tolist()

placeholder = st.empty()
for i in range(20):  # Simulate streaming data updates
    live_df = simulate_health_data(num_users=3, minutes=1)
    live_processed, live_scaled, _ = preprocess_data(live_df)
    live_preds, _ = detect_anomalies(live_scaled, contamination=contamination)
    live_df['anomaly'] = ['Anomaly' if x == -1 else 'Normal' for x in live_preds]
    ...
    with placeholder.container():
        st.subheader(f" ⏱Live Update {i+1}")
        st.dataframe(live_df[['user_id', 'timestamp', 'heart_rate', 'temperature', 'blood_oxygen', 'anomaly']])
        chart = sns.lineplot(data=live_df, x='timestamp', y='heart_rate', hue='user_id')
        fig_rt = chart.get_figure()
        st.pyplot(fig_rt)
        chart.clear()

        if alert_users:
            st.error(f"Alert: Abnormal trends detected for: {', '.join(alert_users)}")
...
# Create Medication Adherence Rate plot
adherence = df.groupby('user_id')['medication_taken'].mean() * 100
fig2, ax2 = plt.subplots()
adherence.plot(kind='bar', ax=ax2)
ax2.set_ylim(0, 100)
ax2.set_ylabel("Adherence Rate (%)")
ax2.set_title("Medication Adherence Rate per User")
st.pyplot(fig2)
